...
def _timeout_handler(signum, frame):
    raise TimeoutError("update_rates.py exceeded safety time limit")
...
def fetch_naver_rub_krw():
    ...
    if (
        (result["naver_rub_krw"] is None or result["naver_rub_krw"] > 100)
        and result["naver_send"]
        and result["naver_receive"]
    ):
        result["naver_rub_krw"] = round(
            (result["naver_send"] + result["naver_receive"]) / 2,
            4,
        )
